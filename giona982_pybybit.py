...
import logging
logging.basicConfig(filename="giona982_pybybit.log", level=logging.DEBUG,
...
session = HTTP(
    testnet=False,
    api_key="nrcbVDmfT90Dn9dEN8",
    api_secret="SgJPH3fT9uoSqnDZJ4HiXvQNqIp3JwTa862Z",
)
...
if len(order) == 0:
    subprocess.Popen([f"python3 giona982_pybybit.py {symbol} {side} {qty} {price} {stopLoss} {takeProfit} {leverage} {tradeMode}"], stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
