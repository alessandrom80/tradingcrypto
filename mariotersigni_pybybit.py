...
import logging
logging.basicConfig(filename="mariotersigni_pybybit.log", level=logging.DEBUG,
...
session = HTTP(
    testnet=False,
    api_key="Byk9SQFaighqTT9bNe",
    api_secret="bd0oYHVMkPBSRUEiRrObl4Wuhfhyoq1NKZt4",
)
...
if len(order) == 0:
    subprocess.Popen([f"python3 mariotersigni_pybybit.py {symbol} {side} {qty} {price} {stopLoss} {takeProfit} {leverage} {tradeMode}"], stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
