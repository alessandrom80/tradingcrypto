...
import logging
logging.basicConfig(filename="sandrotersigni53_pybybit.log", level=logging.DEBUG,
...
session = HTTP(
    testnet=False,
    api_key="8NjFWylWXmE70GOT01",
    api_secret="ANxZnRn80eVMYOhFS1RWfi2aE9l60BcmqE67",
)
...
if len(order) == 0:
    subprocess.Popen([f"python3 sandrotersigni53_pybybit.py {symbol} {side} {qty} {price} {stopLoss} {takeProfit} {leverage} {tradeMode}"], stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
