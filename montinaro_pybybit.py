...
import logging
logging.basicConfig(filename="montinaro_pybybit.log", level=logging.DEBUG,
...
session = HTTP(
    testnet=False,
    api_key="1ahLRW7cNmqSNNWHGa",
    api_secret="mMNqYQl00aNJBsFASTuOU5rvHweCnvbiNuaj",
)
...
if len(order) == 0:
    subprocess.Popen([f"python3 montinaro_pybybit.py {symbol} {side} {qty} {price} {stopLoss} {takeProfit} {leverage} {tradeMode}"], stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
