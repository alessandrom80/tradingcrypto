...
import logging
logging.basicConfig(filename="pepperoscio_pybybit.log", level=logging.DEBUG,
...
session = HTTP(
    testnet=False,
    api_key="ukJWRKv34H68nShjjr",
    api_secret="mxH2KOp5W9KaatVS7jWPUTkuGF2hRLL1UrHX",
)
...
if len(order) == 0:
    subprocess.Popen([f"python3 pepperoscio_pybybit.py {symbol} {side} {qty} {price} {stopLoss} {takeProfit} {leverage} {tradeMode}"], stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
