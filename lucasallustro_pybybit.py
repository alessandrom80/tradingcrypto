...
import logging
logging.basicConfig(filename="lucasallustro_pybybit.log", level=logging.DEBUG,
...
session = HTTP(
    testnet=False,
    api_key="JuFbanXvkqNNz5f5Vs",
    api_secret="4oL8h9e2QR5dP922RT9mSUl8hkRabtt1EUES",
)
...
if len(order) == 0:
    subprocess.Popen([f"python3 lucasallustro_pybybit.py {symbol} {side} {qty} {price} {stopLoss} {takeProfit} {leverage} {tradeMode}"], stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
