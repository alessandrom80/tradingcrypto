...
import logging
logging.basicConfig(filename="ricky_pybybit.log", level=logging.DEBUG,
...
if len(order) == 0:
    subprocess.Popen([f"python3 ricky_pybybit.py {symbol} {side} {qty} {price} {stopLoss} {takeProfit} {leverage} {tradeMode}"], stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
